...
const _cpPrefEngine = require('./cp/cr-preference-engine');
...
class CXClientContext extends _cx_data.DBContext {
    ...
    async init(options) {
        try {
            ...
            if (options.cxSvc) {
                ...
                this.#cpPrefEngine = new _cpPrefEngine.CPPreferenceEngine(this);
    ...
    async getUserSavedFilters(request, appendCacheKey, emptyQuery) {
        if (!emptyQuery) { emptyQuery = {}; }
        var cacheKey = request.path + (appendCacheKey || '');
        var userCache = await this.table(_cxSchema.cx_login_cache).fetch(this.tUserId, _cxConst.CX_LOGIN_CACHE_TYPE.URL_QUERY, cacheKey);
        if (userCache && userCache.cacheKey == cacheKey && JSON.stringify(request.query) == JSON.stringify(emptyQuery)) {
        //if (userCache && userCache.cacheKey == cacheKey && userCache.cacheValue && request.query.saved != 'T') {
            return `${request.path}${userCache.cacheValue}`;

        } else {
            // @@TODO: store shit here ???
            if (JSON.stringify(request.query) == JSON.stringify(emptyQuery)) { return; }
            var search = request.url.search;
            if (!userCache) {
                userCache = this.table(_cxSchema.cx_login_cache).createNew();
                userCache.loginId = this.tUserId;
                userCache.cacheType = _cxConst.CX_LOGIN_CACHE_TYPE.URL_QUERY;
                userCache.cacheKey = cacheKey;
            }
            if (userCache.cacheValue != search) {
                userCache.cacheValue = search;

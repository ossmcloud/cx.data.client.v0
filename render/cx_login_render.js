...
class CXLogin extends RenderBase {
    ...
    async _record() {
        ...
        if (this.options.mode == 'view') {
            if (this.dataSource.cx.roleId >= _cxConst.CX_ROLE.ADMIN) {
                if (this.dataSource.loginId != this.dataSource.cx.tUserId) {
                    ...
                    if (this.dataSource.status == 1) { this.options.buttons.push({ id: 'cx_login', text: 'Lock User', function: 'lockLogin' }); }
                }
            }
        }
    }
    ...
    async _list() {
        ...
        var userStatusLookUps = [
            { value: -1, text: 'Not Verified' },
        ...
        this.options.columns = [
            ...
            { name: 'status', title: 'status', align: 'center', width: '100px', lookUps: userStatusLookUps },
            { name: _cxSchema.cx_login.ROLEID, title: 'role', align: 'center', width: '100px', lookUps: lookUps },
        ...
        this.options.cellHighlights.push({
            column: 'status', op: '=', value: 0,
            style: 'background-color: rgb(127,127,127); color: white; ' + applyStyle,
            columns: ['status']
        });
        this.options.cellHighlights.push({
            column: 'status', op: '=', value: 9,
            style: 'background-color: rgb(246,0,0); color: white; ' + applyStyle,
            columns: ['status']
        });
        this.options.cellHighlights.push({
            column: 'status', op: '=', value: 99,
            style: 'background-color: rgb(127,127,127); color: white; ' + applyStyle,
            columns: ['status']
        });
    }

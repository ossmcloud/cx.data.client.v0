...
class CRPreferenceRender extends RenderBase {
    ...
    async _loadPreferenceRecords(allowEdit) {
        var prefRecords = this.dataSource.cx.table(_cxSchema.cp_preference_record);
        ...
        var listOptions = {
            ...
            highlights: [{
                column: _cxSchema.cp_preference_record.DISABLED, op: '=', value: true,
                //   columns: [_cxSchema.cp_preference_record.RECORDTYPE, _cxSchema.cp_preference_record.LEVELID],
    ...
    async _loadPreferenceConfigs(allowEdit) {
        ...
        if (this.dataSource.type == 'value') {
            valueSql = '(select pv.label from cp_preference_value pv where pv.preferenceValueId = pc.value) as [value]'
        }
        ...
        var query = {
            sql: `  select      pc.preferenceId, pc.preferenceRecordId, pr.levelId, pr.disabled, pr.recordType, pc.recordId, ${valueSql},
                    ...
                    from        ${_cxSchema.cp_preference_config.TBL_NAME} pc
                    inner join  ${_cxSchema.cp_preference_record.TBL_NAME} pr ON pr.preferenceRecordId = pc.preferenceRecordId
                    where       pc.${_cxSchema.cp_preference_config.PREFERENCEID} = @${_cxSchema.cp_preference_config.PREFERENCEID}
                    order by    pr.levelId, recordName`,
            params: [
                { name: _cxSchema.cp_preference_config.PREFERENCEID, value: this.dataSource.id }
            ]
        };
    ...
    async _record() {
        ...
        if (this.dataSource.type == 'value') {
            ...
            var prefValues = this.dataSource.cx.table(_cxSchema.cp_preference_value);
                        ...
                        group: 'main', title: 'main info', columnCount: 1, inline: true, fields: [
                            {
                                group: 'main', columnCount: 2, inline: true, fields: [
                                    { name: _cxSchema.cp_preference.NAME, label: 'name', column: 1 },
                                    { name: _cxSchema.cp_preference.TYPE, label: 'type', column: 2, readOnly: true, lookUps: prefValueTypes },
                                ]
                            },
                            { name: _cxSchema.cp_preference.DESCRIPTION, type: _cxConst.RENDER.CTRL_TYPE.TEXT_AREA, label: 'description', validation: '{ "mandatory": true, "max": 255  }' },
                            { name: _cxSchema.cp_preference.DEFAULTVALUE, label: 'global default', type: controlType, lookUps: valuesLookUp },
    ...
    async _list() {
        this.options.columns = [
            { name: _cxSchema.cp_preference.PREFERENCEID, title: ' ', align: 'center' },
            { name: _cxSchema.cp_preference.NAME, title: 'preference name' },
            { name: _cxSchema.cp_preference.TYPE, title: 'value type', lookUps: prefValueTypes },
            { name: _cxSchema.cp_preference.DEFAULTVALUE, title: 'global default' },

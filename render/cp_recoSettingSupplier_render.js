...
class CPRecoSettingSupplierRender extends RenderBase {
    constructor(dataSource, options) {
        ...
        this.autoLoadFields[_cxSchema.cp_recoSettingSupplier.IGNOREDRS] = null;
    ...
    async initColumn(field, column) {
        if (field.name == _cxSchema.cp_recoSettingSupplier.RECOSETTINGID) {
            ...
            column.nullText = '';
        } else if (field.name == _cxSchema.cp_recoSettingSupplier.IGNOREVATMISMATCH || field.name == _cxSchema.cp_recoSettingSupplier.IGNORELINETOLERANCE
            || field.name == _cxSchema.cp_recoSettingSupplier.IGNORELINES || field.name == _cxSchema.cp_recoSettingSupplier.IGNOREDRS || field.name == _cxSchema.cp_recoSettingSupplier.NODELIVERY) {
            column.align = 'center';
            column.width = '100px';
        }
    }
                ...
                group: 'all', title: '', columnCount: 3, fields: [
                    {
                        group: 'main1', title: 'main settings', column: 1, columnCount: 1, fields: [
                            ...
                            { name: 'ignoreDRS', label: 'ignore DRS when matching', column: 1 },
